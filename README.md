...
:exclamation:注意 `count(>4,3)`和`count(>4.0,3)`是不一样，如果
...
:exclamation:注意 `count(>4,3)`和`count(>4.0,3)`是不一样，如果value的值是浮点则需要使用`count(>4.0,3)`，下面所有的例子都是如此。
...
1. 过滤出想要的数据
2. 限制数据的范围，可以数据个数或者时间
3. 运算规则
4. 符合条件

还是  `filter((-5,5)).limit(5).take(0,2).sum().meet(>1)`举例：
...
- **filter**
  ...
  filter函数放在链式表达式的最前面表示过滤出想要的数据，如果不需要过滤数据则可以不写。filter函数可以传入比较表达式，区间表达式，逻辑表达式。例如：

  > filter(((1,7]||>10)&&!=5) // 还可以使用小括号()来指定运算优先级
  > filter(<=3)
...
- **limit**
  ...
  limit表示数据限制，limit函数可以填写 时间或者个数，例如：

  > limit(100ms) //限制100毫秒内的数据
  >
  > limit(10s) //限制10秒内的数据
  >
  > limit(1m) //限制1分钟内的数据
...
- **take**
  ...
  take函数有两个参数 第一个参数表示起始索引(包含)，第二个参数表示结束索引（不包含）,如果第二个参数为空则默认取到最后一个元素,比如存在一下序列
